...
export default function TradesPage() {
  ...
  const rows = useMemo(() => {
    return MOCK_TRADE_ROWS.filter((r) => {
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <div
            className={`font-mono text-xl tabular-nums ${signTone(netR)}`}
          >
            {formatR(netR)}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Select label="Setup" value={setup} onChange={setSetup} options={["All", ...MOCK_SETUP_NAMES]} />
        <Select
          label="Session"
          value={session}
          ...
          options={SESSIONS}
        />
        <Select label="Symbol" value={symbol} onChange={setSymbol} options={["All", ...MOCK_SYMBOLS]} />
        <Select
          label="Status"
          value={status}
        ...
        <div className="ml-auto flex items-end text-xs text-neutral-600">
          {rows.length} of {MOCK_TRADE_ROWS.length} trades
        </div>
      </div>
